...
const EditTaskModal = ({ isOpen, onClose, onSave, task }: any) => {
    ...
    const [value, setValue] = useState<number>(0); // Usar 'value' en lugar de 'price'

    useEffect(() => {
        if (task) {
            setName(task.name);
            setPriority(task.priority);
            setValue(task.value || 0); // Inicializar con task.value
        }
    }, [task]);
    ...
    const handleSave = () => {
        onSave({ ...task, name, priority, value }); // Enviar 'value' en el payload
    };
            ...
            <div className="bg-[#14171E] rounded-2xl p-8 max-w-md w-full border border-white/10 shadow-2xl relative">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" value={value} onChange={(e) => setValue(parseFloat(e.target.value))} className="bg-[#242933] w-full text-sm rounded-md px-3 py-2 mt-1 text-[#E0E3E8] focus:outline-none focus:ring-2 focus:ring-[#00ADB5]" />
                    </div>
                </div>
