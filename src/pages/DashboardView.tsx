...
import { Briefcase, Rocket, Home, DollarSign, Plus, Edit2, CheckCircle, Trash2, Play, Pause } from 'lucide-react';
import { InboxTask, FocusTask, Client, Project } from '../types';
import InboxWidget from '../components/InboxWidget';
import ComboBox from '../components/ComboBox';

const DashboardView = ({ clients, projects, focusTasks, onUpdateTasks, inboxTasks, handlers, timerState, taskToPlan }: any) => {
   const { activeTaskId, timer, pomodoroDuration, setPomodoroDuration } = timerState;
   ...
   const resetForm = useCallback(() => {
       ...
       handlers.clearTaskToPlan();
   }, [handlers]);

   useEffect(() => { if (taskToPlan) setPlanningName(taskToPlan.name); }, [taskToPlan]);
   ...
   const handleCommit = (e: React.FormEvent) => {
       ...
       handlers.handleCommitToFocus(taskData);
       resetForm();
   };
   ...
   const handleDrop = (e: React.DragEvent, context: 'billable' | 'internal' | 'personal' | null = null) => {
       ...
       if (taskJSON) {
           ...
           handlers.handleSelectTaskToPlan(task);
           if (context) {
   ...
   return (
       <>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
               <InboxWidget inboxTasks={inboxTasks} onAddTask={handlers.handleAddToInbox} onPlanTask={handlers.handleSelectTaskToPlan} onDeleteTask={handlers.handleDeleteFromInbox} />
               <div 
               ...
               >
                   ...
                   {!focusType && !taskToPlan ? (<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                       ...
                       <form onSubmit={handleCommit} className="space-y-3">
                           ...
                           {focusType === 'billable' && (<div className="flex flex-col md:flex-row gap-2"><ComboBox items={clients} value={planningClient} onChange={setPlanningClient} onCreate={handlers.handleCreateClient} placeholder="Cliente" className="w-full md:w-1/2" /><ComboBox items={projects.filter((p: Project) => p.clientId === clients.find((c: Client) => c.name === planningClient)?.id)} value={planningProject} onChange={setPlanningProject} onCreate={(name: string) => handlers.handleCreateProject(name, clients.find((c: Client) => c.name === planningClient)?.id)} placeholder="Proyecto" disabled={!planningClient} className="w-full md:w-1/2" /></div>)}
                           {focusType === 'internal' && <ComboBox items={projects.filter((p: Project) => p.clientId === -1)} value={planningProject} onChange={setPlanningProject} onCreate={(name: string) => handlers.handleCreateProject(name, -1)} placeholder="Proyecto Interno" />}
                    ...
                    <div className="flex justify-between items-center mt-6 mb-2"><h4 className="text-sm font-semibold text-white">Tareas para Hoy</h4><div className="flex items-center gap-2 text-sm bg-[#242933] p-1 rounded-md">{[5, 15, 25].map(min => <button key={min} onClick={() => setPomodoroDuration(min * 60)} className={`px-2 py-1 rounded ${pomodoroDuration === min * 60 ? 'bg-[#00ADB5] text-black font-bold' : 'hover:bg-white/10'}`}>{min}m</button>)}<button onClick={handlers.handleOpenCustomTimeModal} className={`px-2 py-1 rounded ${isCustomTimeActive ? 'bg-[#00ADB5] text-black font-bold' : 'hover:bg-white/10'}`}>{isCustomTimeActive ? `${pomodoroDuration / 60}m` : '...'}</button></div></div>
                   <div className="space-y-2 max-h-72 overflow-y-auto pr-2 mt-4">
                       {focusTasks.length === 0 && <p className="text-center text-gray-500 py-8">Arrastra una tarea del buzón o crea una nueva para empezar.</p>}
                       {focusTasks.sort((a: FocusTask, b: FocusTask) => { const p = { high: 0, medium: 1, low: 2 }; return p[a.priority] - p[b.priority]; }).map((task: FocusTask) => {
                           ...
                           return (
                               <div key={task.id} className={`group relative bg-[#242933] p-3 rounded-lg flex items-center justify-between transition-all overflow-hidden ${isTaskActive ? 'ring-2 ring-[#00ADB5]' : ''}`}>
                                   ...
                                   <div className="relative z-10 flex items-center gap-3"><PriorityIndicator priority={task.priority} /><button onClick={() => handlers.handleToggleComplete(task.id)} className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center border-2 ${task.completed ? 'bg-[#00ADB5] border-[#00ADB5]' : 'border-[#6C7581]'}`}>{task.completed && <CheckCircle size={14} className="text-white"/>}</button><div><p className={`font-medium ${task.completed ? 'text-[#6C7581] line-through' : 'text-[#E0E3E8]'}`}>{task.name}</p><p className="text-xs text-[#6C7581]">{client?.name ? `${client.name} / ${project?.name}` : project?.name || "Personal"}</p></div></div>
                                   <div className="relative z-10 flex items-center gap-2"><span className="text-sm font-semibold text-white/80">{task.value > 0 ? `$${task.value}` : ''}</span><span className="text-sm font-semibold text-white">{task.pomodoros}</span>
                                       <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                           <button onClick={() => handlers.handleStartEdit(task)} disabled={isTaskActive} className="text-[#6C7581] hover:text-white disabled:opacity-20"><Edit2 size={16} /></button>
                                           <button onClick={() => handlers.handleDeleteTask(task.id)} className="text-[#6C7581] hover:text-red-500"><Trash2 size={16} /></button>
                                       </div>
                                       <button onClick={() => handlers.handleToggleTimer(task.id)} className="text-[#00ADB5] hover:text-[#00c5cf]">{isTaskActive ? <Pause size={20} /> : <Play size={20} />}</button>
                                   </div>
                               </div>
                           );
                       })}
